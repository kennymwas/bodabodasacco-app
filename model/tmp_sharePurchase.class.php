<?php
class tmp_sharePurchase extends db{
	public $id = 0;
	public $member_id = 0;
	...
	public function __construct($id=''){
		parent::__construct();
		if(!empty($id)){
			$rs = $this->dbSelect_where('tmp_share_purchase', '*', array('share_purchase_id' => $id));
			if(empty($rs)){
	...
	public function delete_me(){
		if(!empty($this->id)){
			$this->dbDelete('tmp_share_purchase',array('share_purchase_id' => $this->id));
		}else{
	...
	public function get_share_purchase_data_array($id){
		if(!empty($id)){
			$mbr = $this->dbSelect_where('tmp_share_purchase', '*', array('member_id' => $id));
			if(empty($mbr)){
				return array();
			}else{
				return $mbr;
	...
	public function save(){
		if($this->id==null){
			$inst =  $this->dbInsert('tmp_share_purchase', 
			...
			$this->dbUpdate('tmp_share_purchase', 
