<?php
class shareTransaction extends db{
	...
	public function get_debit_account_transaction_charge(){
		return $this->registry->sacco_configs->debit_share_trans_charge;
	}

	public function get_credit_account_transaction_charge(){
		return $this->registry->sacco_configs->credit_share_trans_charge;
	}
	...
	public function effect_transaction(){
		...
		$inst = $this->dbInsert('tmp_share_transaction',$val);
	...
	private function trans_fee_charge(account $mem_account, $amount, $trans_id){
		if($amount<0) throw new Exception ('Share Trans-charge not effected, but share-transactions have been effected. Invalid trans-charge supplied');
		$mem_account->reduce_account_bal($amount);
		$this->dbInsert('share_trans_charges', array('trans_id'=>$trans_id, 'account_id'=>$mem_account->id, 'share_trans_charge_amount'=>$amount));
	}
}