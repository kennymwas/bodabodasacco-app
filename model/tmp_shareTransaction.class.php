<?php
class tmp_shareTransaction extends db{
	...
	public $share_trans_charge;	
	
	private $amount;

	public function __construct($id){
		parent::__construct();
		$rs = $this->dbSelect_where('tmp_share_transaction', '*', array('tmp_share_transaction_id' => $id));
...
//		echo $this->tmp_share_transaction_id;exit;
		if(!empty($this->tmp_share_transaction_id)){
			$rs = $this->dbSelect_where('share_trans_charges', '*', array('trans_id' => $this->tmp_share_transaction_id));
			return $rs;
		}else{
			throw new Exception('Temp Share Transaction data not loaded yet. Cannot execute get_debit_transaction_charge()');
		}
	}
	...
	public function effect_transaction(){
		if(!empty($this->tmp_share_transaction_id)){
			...
			$ins = $this->dbInsert('share_transaction',$val);
			$this->dbDelete('tmp_share_transaction' , array('tmp_share_transaction_id' => $this->tmp_share_transaction_id));
			
			$ent = new entityData($sender->id);
			$ent->set_last_share_trans_id($ins['last_insert_id']);
