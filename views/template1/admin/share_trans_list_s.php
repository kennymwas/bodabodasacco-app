					...
					<table width="237" border="0" cellspacing="0" cellpadding="26">
						<tr>
							<td>
								...
								[<a href="<?php echo $admin_url; ?>share_trans/list_p">Pending share transfers</a>]<br/><br/>
										...
										<td>
											...
											<?php
												if(isset($no_page_info)){
													...
													echo '<td><strong>Credited Member</strong></td>';
													echo '<td><strong>Debited Member</strong></td>';
													echo '<td><strong>Transaction Date</strong></td>';
													echo '<td><strong>Share Amount</strong></td>';
													echo '<td><strong>Value/Share</strong></td>';
													...
													foreach($trans_list as $row_data){
														...
														echo '<td>'.$row_data['share_transaction_id'].'</td>';
														echo '<td>'.$row_data['cred_name'].'</td>';
														echo '<td>'.$row_data['deb_name'].'</td>';
														echo '<td>'.$row_data['share_transaction_date'].'</td>';
														echo '<td>'.$row_data['share_transaction_amount'].'</td>';
														echo '<td>'.$row_data['share_value'].'</td>';
														echo '<td>'.$site_currency.$row_data['share_value']*$row_data['share_transaction_amount'].'</td>';
														echo "<td>[<a href='".$admin_url."share_trans/approve_sale/".$row_data['share_transaction_id']."'>approve</a>] | [<a href='".$admin_url."share_trans/cancel_sale/".$row_data['share_transaction_id']."'>cancel</a>]</td>";
														echo "</tr>";
													}
													echo '</table><br/><br/>';
													if($pageNum==1){
														...
														echo'[  <a href=\''.$admin_url.'share_trans/list_s/page/1\'> f i r s t </a> ] .:|:. ';
													}
													if($pageNum==1){
														echo" p r e v i o u s  .:|:. ";
													}else{
														echo'[ <a href=\''.$admin_url.'share_trans/list_s/page/'.($pageNum-1).'\'> p r e v i o u s </a> ] .:|:. ';
													}
													if($pageNum==$site_page_count){
														echo" n e x t  .:|:. ";
													}else{
														echo'[ <a href=\''.$admin_url.'share_trans/list_s/page/'.($pageNum+1).'\'> n e x t </a> ] .:|:. ';
													}
													if($pageNum==$site_page_count){
														echo" l a s t <br><br>";
													}else{
														echo'[ <a href=\''.$admin_url.'share_trans/list_s/page/'.$site_page_count.'\'> l a s t </a> ]<br><br>';
													}
